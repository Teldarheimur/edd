...
pub fn analysis<L: Hash + Eq + Clone, R: Eq + Clone, I: Ins<R, L>>(lines: &[I]) -> (Kill<R>, In<R>, Out<R>) {
    ...
    let kill_sets: Kill<R> = lines.iter().map(|line| SmallSet::from_iter(line.get_kill().into_iter())).collect();
...
fn liveness_iteration<V: Eq + Clone>(mut in_set: In<V>, mut out_set: Out<V>, succ_sets: &Vec<Vec<usize>>, gen_sets: &Vec<Vec<V>>, kill_sets: &Kill<V>) -> (In<V>, Out<V>) {
    ...
    for (i, (o, ((gen, kill), successors))) in out_set.iter_mut().zip(gen_sets.iter().zip(kill_sets).zip(succ_sets.iter())).enumerate().rev() {
        *o = successors
        ...
        let gen_set = SmallSet::from_iter(gen.iter().map(|v| v.clone()));
