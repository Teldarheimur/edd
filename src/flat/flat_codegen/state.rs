...
use crate::flat::{ticker::Ticker, FlatType, Function, Global, Ident, Label, Line, StaticDecl, Temp};
...
impl<'a> FlattenState<'a> {
    pub fn new(
        ...
        Self {
            local_types: iter::once(None).chain(function.arg_types.iter().map(|t| Some(t.clone()))).collect(),
            symtab: function.local_names.iter().skip(1).zip(1..).map(|(n, i)| ((**n).into(), Temp(i))).collect(),
            fn_name,
            function,
            statics,
            fns,
    ...
    pub fn new_temp(&mut self, name: &str, t: FlatType) -> Temp {
        ...
        self.function.local_names.push(format!("__{name}").into_boxed_str());
        ...
        match var.into() {
            ...
            Ident::Global(g) => {
                if let Some(f) = self.fns.get(&g) {
                    return Some(FlatType::FnPtr(f.arg_types.clone(), Box::new(f.ret_type.clone())));
                } else if &**g.inner() == self.fn_name {
                    return Some(FlatType::FnPtr(self.function.arg_types.clone(), Box::new(self.function.ret_type.clone())));
                }
